<script lang="ts">
  ...
  interface PlanStageSummaryItem {
    stageName: string;
  ...
  function parsePlanStageSummary(content: string): PlanStageSummaryItem[] | null {
    const trimmed = content.trim();
    if (!trimmed.startsWith("{")) {
      return null;
    }
    ...
    const stages = candidate.stages
      ...
      .filter((item): item is PlanStageSummaryItem => item !== null);
  ...
  async function streamMessage(options: {
    ...
    try {
      const response = await fetch(
        `/api/stacks/${data.stack.id}/plan/message/stream`,
        {
          method: "POST",
      ...
      while (true) {
        ...
        while (buffered.includes("\n\n")) {
          ...
          if (result.done) {
            ...
            if (
              result.done.autoSavedPlanPath &&
              result.done.autoSavedStageConfigPath
            ) {
  ...
  async function savePlan(): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/stacks/${data.stack.id}/plan/save`, {
        method: "POST",
      });
...
<main class="stacked-shell mx-auto w-full max-w-5xl px-4 py-5 sm:px-6 sm:py-6">
  <div class="stacked-fade-in">
    <div
      class="mb-6 flex flex-wrap items-center justify-between gap-3 border-b stacked-divider pb-4"
    >
      <div>
        <a
          href={resolve(`/stacks/${data.stack.id}`)}
          class="stacked-link text-sm font-semibold">Return to feature</a
        >
        <h1 class="mt-2 text-3xl font-semibold tracking-tight">
          Planning Chat - {data.stack.name}
        </h1>
        <p class="mt-1 text-sm stacked-subtle">
          Talk through requirements, lock the implementation plan, then ship by
          stages.
        </p>
        <div class="mt-3">
    ...
    <div>
      ...
      >
        {#if messages.length === 0 && !sending}
          ...
          >
            ...
            <p>
              Start by describing what you want to ship. Ask questions and
              iterate until the plan is clear.
            </p>
          </div>
        {:else}
          <div class="space-y-3">
            {#each messages as message (message.id)}
              ...
              {@const messageQuestionAnswers =
                message.role === "user"
                  ...
                  : null}
              {@const messagePlanStageSummary =
                message.role === "assistant"
                  ? parsePlanStageSummary(message.content)
                  : null}
              <div
              ...
              >
                    ...
                    {#each messageQuestionDialog.questions as question, questionIndex (`${question.header}-${question.question}-${questionIndex}`)}
                      ...
                      >
                        ...
                        {#if question.allowCustom}
                          <p class="mt-3 stacked-subtle text-xs">
                ...
                {:else if messagePlanStageSummary}
                  <div class="space-y-2">
                    <p class="stacked-subtle text-xs uppercase tracking-wide">
                      Stages
                    </p>
                    {#each messagePlanStageSummary as stage, stageIndex (`${stage.stageName}-${stageIndex}`)}
                      <p class="leading-snug">
      ...
      {#if session.savedPlanPath}
        <div
          class="mt-3 rounded-lg border border-[var(--stacked-border-soft)] bg-[var(--stacked-bg-soft)] p-3"
        >
          <p class="mb-1 text-[11px] uppercase tracking-wide stacked-subtle">
            Saved plan path
          </p>
          <p
            class="stacked-chat-font break-all text-sm text-[var(--stacked-text)]"
          >
            {session.savedPlanPath}
          </p>
          {#if session.savedStageConfigPath}
            <p
              class="mb-1 mt-3 text-[11px] uppercase tracking-wide stacked-subtle"
            >
              Saved stage config path
            </p>
            <p
              class="stacked-chat-font break-all text-sm text-[var(--stacked-text)]"
            >
