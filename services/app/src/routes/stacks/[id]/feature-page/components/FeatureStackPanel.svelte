<script lang="ts">
  ...
  function hasOutOfSyncStages(): boolean {
    return (stack.stages ?? []).some(
      (stage) => stageSyncMetadata(stage.id).isOutOfSync,
    );
  }
  ...
  function stagePullRequest(
    stageId: string,
    fallback?: StackPullRequest,
  ): StackPullRequest | undefined {
    return resolveStagePullRequest(
      implementationRuntimeByStageId,
      stageId,
      fallback,
    );
  ...
  function implementationStageColor(
  ...
  async function refreshImplementationRuntime(): Promise<void> {
    ...
    const entries = await Promise.all(
      stageIds.map(async (stageId) => {
        const stageEntry = (stack.stages ?? []).find(
          (stage) => stage.id === stageId,
        );
        const fallbackStatus = stageEntry
          ? stageStatus(stageId, stageEntry.status)
          : 'in-progress';

        try {
          const response = await fetch(
            `/api/stacks/${stack.id}/stages/${stageId}/implementation/status`,
          );
          const payload =
            (await response.json()) as ImplementationStatusResponse;
          if (!response.ok) {
            throw new Error(
              payload.error ?? 'Unable to load implementation status.',
            );
          }
  ...
  async function startFeature(): Promise<void> {
    ...
    try {
      ...
      const stageLabel = body.stageNumber
        ...
        : 'next stage';
      const mode = body.startedNow
        ? `Started ${stageLabel}.`
        : `${stageLabel} is already running.`;
      const worktreeState = body.reusedWorktree
        ? 'Reused existing worktree.'
        : 'Created worktree.';
      const sessionState = body.reusedSession
  ...
  async function syncStack(): Promise<void> {
    ...
    try {
      const response = await fetch(`/api/stacks/${stack.id}/sync`, {
        method: 'POST',
      });
      ...
      syncError =
        error instanceof Error ? error.message : 'Unable to sync stack.';
    } finally {
      syncPending = false;
    }
  }
    ...
    {#if stack.stages && stack.stages.length > 0}
      <div class="space-y-2">
        {#each stack.stages as implementationStage (implementationStage.id)}
          {@const stageRuntime =
            implementationRuntimeByStageId[implementationStage.id]}
          {@const currentStageStatus = stageStatus(
            implementationStage.id,
            implementationStage.status,
          )}
          ...
          {@const stageWorking =
            ...
            isStageAgentWorking(implementationStage.id)}
  ...
  <div>
    {#if stack.pullRequest}
      ...
      <p class="mt-1 text-sm font-semibold text-[var(--stacked-text)]">
        #{stack.pullRequest.number}
        {stack.pullRequest.title}
      </p>
      <p class="mt-1 text-xs stacked-subtle">
