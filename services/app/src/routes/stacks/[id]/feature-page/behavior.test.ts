...
describe('feature page behavior contracts', () => {
	it('maps stage statuses to legacy labels and class names', () => {
		...
		expect(implementationStageClass('in-progress')).toBe('stacked-chip stacked-chip-warning');
		expect(implementationStageClass('review-ready')).toBe('stacked-chip stacked-chip-review');
		expect(implementationStageClass('done')).toBe('stacked-chip stacked-chip-success');
	});
	...
	it('resolves runtime stage status and pull request with fallback values', () => {
		...
		expect(stageStatus(runtimeByStageId, '1', 'in-progress')).toBe('review-ready');
		expect(stageStatus(runtimeByStageId, 'missing', 'in-progress')).toBe('in-progress');
		expect(stagePullRequest(runtimeByStageId, '1', undefined)?.number).toBe(12);
		expect(stagePullRequest(runtimeByStageId, 'missing', undefined)).toBeUndefined();
	});
	...
	it('keeps start action gating parity with page behavior', () => {
		...
		expect(canStartFeature({ stages: [...stages], implementationRuntimeByStageId: {}, startPending: false })).toBe(
			true
		);
		expect(startButtonLabel({ stages: [...stages], implementationRuntimeByStageId: {}, startPending: false })).toBe(
			'Next stage'
		);

		expect(canStartFeature({ stages: [...stages], implementationRuntimeByStageId: {}, startPending: true })).toBe(
			false
		);
		expect(startButtonLabel({ stages: [...stages], implementationRuntimeByStageId: {}, startPending: true })).toBe(
			'Starting...'
		);
