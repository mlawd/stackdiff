...
import { getStackById, touchPlanningSessionUpdatedAt } from '$lib/server/stack-store';
...
function isDebugEnabled(): boolean {
	return process.env.OPENCODE_DEBUG === '1' || process.env.OPENCODE_DEBUG === 'true';
}
			...
			async start(controller) {
				try {
					...
					const streamEvents = parsed.watch
						...
						: streamOpencodeSessionMessage(session.opencodeSessionId as string, content, {
