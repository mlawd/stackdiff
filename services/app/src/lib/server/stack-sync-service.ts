...
import type { StageSyncMetadata, StackImplementationSession, StackMetadata } from '$lib/types/stack';

export type StackSyncErrorCode = 'not-found' | 'invalid-state' | 'command-failed';
...
async function resolveExistingRef(repositoryRoot: string, ref: string): Promise<string> {
	if (ref.startsWith('origin/')) {
		const remoteRef = await runCommand('git', ['rev-parse', '--verify', '--quiet', `${ref}^{commit}`], repositoryRoot);
		if (remoteRef.ok) {
			return ref;
		}
	...
	const localRef = await runCommand('git', ['rev-parse', '--verify', '--quiet', `${ref}^{commit}`], repositoryRoot);
	if (localRef.ok) {
		return ref;
	}
...
async function countBehind(repositoryRoot: string, branchName: string, baseRef: string): Promise<number> {
	const count = await runCommand('git', ['rev-list', '--count', `${branchName}..${baseRef}`], repositoryRoot);
	...
	if (!Number.isFinite(parsed) || parsed < 0) {
		throw new StackSyncServiceError('command-failed', `Unable to parse behind count for ${branchName}.`);
	}
	...
	for (let index = 0; index < stages.length; index += 1) {
		...
		if (!session?.branchName) {
			contexts.push({
				...
				reasonIfUnavailable: 'Stage branch is unavailable. Start this stage first.'
			});
			continue;
		}
		...
		if (index === 0) {
			...
			const resolvedBaseRef = await resolveExistingRef(repositoryRoot, defaultBaseBranch);
			contexts.push({
				stageId: stage.id,
				stageTitle: stage.title,
				session,
		...
		if (!previousStage) {
			contexts.push({
				...
				session,
				branchName: session.branchName,
				reasonIfUnavailable: 'Previous stage branch is unavailable for sync baseline.'
			});
			continue;
		}
...
export async function getStageSyncById(stack: StackMetadata): Promise<Record<string, StageSyncMetadata>> {
	...
	const contexts = await resolveStageContexts(stack, repositoryRoot, sessionsByStageId);
			...
			if (!context.branchName || !context.baseRef) {
				return [
					...
					stageUnavailableMetadata({
						...
						reason: context.reasonIfUnavailable ?? 'Stage sync status is unavailable.'
					})
				] as const;
			}

			const behindBy = await countBehind(repositoryRoot, context.branchName, context.baseRef);
			return [
				context.stageId,
				{
...
export async function syncStack(stackId: string): Promise<SyncStackResult> {
	...
	for (const context of contexts) {
		...
		const worktreeAbsolutePath = resolveWorktreeAbsolutePath(repositoryRoot, context.session.worktreePathKey);
		...
		if (!rebased.ok) {
			await runCommand('git', ['rebase', '--abort'], worktreeAbsolutePath, { timeoutMs: 120_000 });
			throw new StackSyncServiceError(
				'command-failed',
...
export function isStackSyncServiceError(error: unknown): error is { code: StackSyncErrorCode; message: string } {
