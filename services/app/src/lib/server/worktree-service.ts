...
function sanitizeToken(value: string, fallback: string, maxLength: number): string {
...
function toWorktreePathKey(absolutePath: string, repositoryRoot: string): string {
...
async function listGitWorktrees(repositoryRoot: string): Promise<ParsedWorktree[]> {
	const listed = await runCommand('git', ['worktree', 'list', '--porcelain'], repositoryRoot);
	if (!listed.ok) {
		throw new Error(`Unable to inspect git worktrees: ${listed.stderr || listed.error || 'unknown failure'}`);
	}
...
export function resolveWorktreeAbsolutePath(repositoryRoot: string, worktreePathKey: string): string {
...
export async function resolveDefaultBaseBranch(repositoryRoot: string): Promise<string> {
	...
	const remoteShow = await runCommand('git', ['remote', 'show', 'origin'], repositoryRoot);
	if (remoteShow.ok) {
	...
	throw new Error('Unable to resolve the repository default base branch. Configure origin/HEAD or create main/master.');
}
...
export async function ensureStageBranchWorktree(input: {
	...
	const worktreeAbsolutePath = resolveWorktreeAbsolutePath(input.repositoryRoot, input.worktreePathKey);
	const worktrees = await listGitWorktrees(input.repositoryRoot);
	const targetBranchRef = `refs/heads/${input.branchName}`;
	const branchAttached = worktrees.find((entry) => entry.branchRef === targetBranchRef);
	...
	if (branchAttached) {
		return {
			...
			worktreePathKey: toWorktreePathKey(branchAttached.path, input.repositoryRoot),
			reusedWorktree: true
		};
	}

	const worktreePathUsed = worktrees.find((entry) => path.resolve(entry.path) === path.resolve(worktreeAbsolutePath));
	if (worktreePathUsed && worktreePathUsed.branchRef && worktreePathUsed.branchRef !== targetBranchRef) {
		throw new Error(
	...
	const branchExists = await branchRefExists(input.repositoryRoot, input.branchName);
	const args = branchExists
		? ['worktree', 'add', worktreeAbsolutePath, input.branchName]
		: ['worktree', 'add', '-b', input.branchName, worktreeAbsolutePath, input.baseBranch];
