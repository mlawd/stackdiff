...
function isStackMergeCheckStatus(
  value: unknown,
): value is StackMergeCheckStatus {
  return (
    value === "pending" ||
    value === "passed" ||
    value === "failed" ||
...
function isStackMergeCheckSource(
  value: unknown,
): value is StackMergeCheckSource {
  return value === "git" || value === "gh" || value === "stacked";
}

function isStackMergeCheckResult(
  value: unknown,
): value is StackMergeCheckResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
...
function isStackMergeChecksSummary(
  value: unknown,
): value is StackMergeChecksSummary {
  if (typeof value !== "object" || value === null) {
    return false;
  }
...
function isStackMergeReadinessBlockerCode(
  value: unknown,
): value is StackMergeReadinessBlockerCode {
  return (
    value === "NO_STAGES" ||
...
function isStackMergeReadinessBlocker(
  value: unknown,
): value is StackMergeReadinessBlocker {
  if (typeof value !== "object" || value === null) {
    return false;
  }
...
function isStackMergeResultStatus(
  value: unknown,
): value is StackMergeResultStatus {
  return value === "merged" || value === "blocked" || value === "failed";
}

function isStackMergeBranchDetail(
  value: unknown,
): value is StackMergeBranchDetail {
  if (typeof value !== "object" || value === null) {
    return false;
  }
...
function isStackMergeResult(value: unknown): value is StackMergeResult {
  ...
  return (
    ...
    (result.mergeCommitSha === undefined ||
      typeof result.mergeCommitSha === "string") &&
...
function isStackMergeMetadata(value: unknown): value is StackMergeMetadata {
  ...
  return (
    (metadata.lastReadiness === undefined ||
      isStackMergeReadiness(metadata.lastReadiness)) &&
    (metadata.lastMergeResult === undefined ||
      isStackMergeResult(metadata.lastMergeResult)) &&
    (metadata.lastMergedAt === undefined ||
      typeof metadata.lastMergedAt === "string") &&
    (metadata.lastMergeCommitSha === undefined ||
      typeof metadata.lastMergeCommitSha === "string") &&
    (metadata.defaultCommitType === undefined ||
      isMergeCommitType(metadata.defaultCommitType))
  );
}
...
function isStackMetadata(value: unknown): value is StackMetadata {
  ...
  return (
    ...
    (stack.stages === undefined ||
      (Array.isArray(stack.stages) && stack.stages.every(isFeatureStage))) &&
...
function isStackFile(value: unknown): value is StackFile {
  ...
  return (
    ...
    (file.planningSessions === undefined ||
      (Array.isArray(file.planningSessions) &&
...
function isStackPlanningSession(value: unknown): value is StackPlanningSession {
  ...
  return (
    ...
    (session.savedPlanPath === undefined ||
      typeof session.savedPlanPath === "string") &&
    (session.savedStageConfigPath === undefined ||
      typeof session.savedStageConfigPath === "string") &&
    (session.savedAt === undefined || typeof session.savedAt === "string")
  );
}

function isStackImplementationSession(
  value: unknown,
): value is StackImplementationSession {
  if (typeof value !== "object" || value === null) {
    return false;
  }
...
export async function getPlanningSessionByStackId(
  id: string,
): Promise<StackPlanningSession | undefined> {
  const file = await readStackFile();
...
export async function getImplementationSessionsByStackId(
  stackId: string,
): Promise<StackImplementationSession[]> {
  const file = await readStackFile();
  return (file.implementationSessions ?? []).filter(
    session => session.stackId === stackId,
  );
}
...
export async function createOrGetPlanningSession(
  ...
  const file = await readStackFile();
  const existing = file.planningSessions?.find(
...
export async function setPlanningSessionOpencodeId(
  ...
  const session = file.planningSessions?.find(
    candidate => candidate.stackId === id,
  );
...
export async function touchPlanningSessionUpdatedAt(
  id: string,
): Promise<StackPlanningSession> {
  const file = await readStackFile();
  const session = file.planningSessions?.find(
...
export async function deleteStack(id: string): Promise<void> {
  ...
  await writeStackFile({
    ...file,
    stacks: nextStacks,
    planningSessions: (file.planningSessions ?? []).filter(
      session => session.stackId !== id,
    ),
...
export async function setStackStages(
  id: string,
  stages: FeatureStage[],
): Promise<StackMetadata> {
  const file = await readStackFile();
